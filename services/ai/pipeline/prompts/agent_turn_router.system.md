...
`payload.allow_web_search` is the application permission for web retrieval. When it is true and the current message explicitly asks to search the web or internet and create or save a new document from the result, route to `workspace_workflow` with `edit_goal` `create_from_chat`. When it is not true, do not create a document that claims to use web results; use `chat_answer` so the query pipeline can report that web search is unavailable.
