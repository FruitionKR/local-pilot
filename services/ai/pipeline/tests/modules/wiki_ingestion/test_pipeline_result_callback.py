...
from app.modules.wiki_ingestion.infrastructure.pipeline_result_callback import (
    ...
    resolve_internal_callback_token,
)
...
def test_result_callback_sends_internal_token_header() -> None:
    requests = []
    ...
    assert requests[0].get_header("X-internal-token") == "secret-token"


def test_result_callback_does_not_retry_unauthorized() -> None:
    attempts = 0
    ...
    with pytest.raises(PipelineResultCallbackError) as error:
        post_pipeline_result(
            "http://backend/result",
            {"operation_id": "op-1"},
            token="wrong-token",
...
def test_internal_callback_token_is_required(monkeypatch) -> None:
