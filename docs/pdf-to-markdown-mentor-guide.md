...
실제 PDF 4개, 총 30페이지로 확인한 핵심 결론은 세 가지다.
...
flag를 켰다고 모든 block이 모델에 들어가는 것은 아니다.
...
이 실험으로 참고문헌 포함 여부는 확인했지만, 제목·일반 본문·그림까지 같은 기준으로 전수 대조한 전체 문서 정확도는 아직 측정하지 않았다.
...
현재 evaluator는 괄호와 Markdown 열 개수 같은 형식은 확인하지만, 원본 숫자와 행·열이 얼마나 보존됐는지는 충분히 확인하지 못한다. 참고문헌 crop에 `[4]`가 보이는데 Vision SLLM 전사에서 번호가 빠져도 나머지 문장이 정상적이면 통과한 사례도 있었다.
...
### 1순위: evaluator의 원본 정보 보존 검사 강화

목록 번호, 숫자, 연도, DOI, 표의 행·열, 수식의 계수·부호·분모가 후보 결과에 남아 있는지 원본 근거와 비교한다. 현재 evaluator가 형식상 정상인 누락을 통과시키는 문제가 가장 직접적인 개선 대상이다.
...
PDF 4개의 표·수식 70개는 모두 사람이 구조를 구분하고 읽을 수 있었다. 다만 원본 확인 없이 정확한 값과 수식을 재사용할 수 있는 수준은 55/70개였으며, 나머지 15개에는 중요한 행·항·값 문제가 있었다. 참고문헌은 91개 모두 포함됐고, 89개는 내용 누락 없이 복원됐다.
...
- 현재 30페이지 전체의 제목·본문·표·수식·그림을 합친 사람 기준 정확도
