...
본문 저장은 Mongo 트랜잭션(본문+revision+write-id+outbox) 후 outbox publisher가 Kafka `document.edit.event`(key=document_id)를 발행한다. ingest는 core DB의 업무 상태·`run_id`·`ai_command_outbox`를 한 트랜잭션에 저장한 뒤 publisher가 Kafka `ai.ingest.command`(key=workspace_id)를 발행한다. 둘 다 at-least-once이며 AI worker는 기존 `run_id` 재전달을 실행하지 않는다. ingest topic은 12 partitions, KEDA lag 기반 스케일(min1/max4)을 사용한다. 결정 근거: [adr/0003](adr/0003-choose-event-processing-strategy.md)
