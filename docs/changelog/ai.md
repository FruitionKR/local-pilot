...
- 추가 리뷰에서 AI 복원 실패 시 AnyDoc 원문을 보존하고 redacted 본문만 CMap 손상 검사에 사용하도록 수정했으며, Heron `Cargo.lock`과 `--locked` 설치를 추가했다. Rust 1.88 locked build와 전체 테스트 `823 passed, 61 subtests passed`로 검증했다.
