...
- 이슈 문서를 날짜별로 관리할 때 지난 날짜 문서에 새 이슈가 남거나, 상세 계약이 다른 문서에만 남아 추적이 어려워질 수 있었다.
- 다른 문서에 구체적인 API 계약이나 정책을 작성하는 경우 이슈 문서에서도 해당 위치를 함께 남기는 규칙이 필요했다.

**변경된 내용**

- `AGENTS.md`와 `CLAUDE.md`에 이슈 문서는 항상 작업 당일 날짜 기준의 `docs/issue/YYYY-MM-DD.md`에 작성하거나 갱신한다는 규칙을 추가했다.
- 다른 문서에 구체적인 계약, 설계, 절차, 정책을 적는 경우 이슈 문서에도 문서 경로와 확인 가능한 위치를 함께 기록하도록 명시했다.
- 지난 날짜 이슈를 새 날짜 기준으로 다시 관리해야 할 때는 새 문서로 옮기고 기존 문서에는 이동 안내와 링크만 남기도록 정리했다.

**검증 결과**

- `rg -n "이슈 문서 관리|docs/issue/YYYY-MM-DD|확인 가능한 위치" AGENTS.md CLAUDE.md docs/changelog/frontend.md`로 지침 반영 위치를 확인했다.
