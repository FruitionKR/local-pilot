...
- 관련: [ADR-0001](0001-choose-primary-database.md), [data-model.md](../data-model.md), `services/backend/document-svc/src/main/resources/db/migration/V25__add_agent_skill_and_langgraph_schema.sql`
...
1. **Skill 테이블 소유권은 Python pipeline 구현을 채택한다.** DDL 생성만 document-svc Flyway가 담당하고(운영 중 pipeline은 DDL을 실행하지 않는다), 읽기·쓰기는 `services/ai/pipeline/app/modules/skill/`이 전담한다. 통합 시 `dev`의 Java `fruition.skill` 구현은 가져오지 않는다.
