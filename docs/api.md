...
| POST | `.../ai-operation-logs/{op}/restore` | 문서 편집 복구는 즉시 완료되어 200, Wiki 복구는 queued 상태로 등록되어 202. `preview_token` 필수, 대상 변경 시 409 `RESTORE_PREVIEW_STALE`, 같은 토큰 재실행은 기존 요청을 만들지 않고 400 `INVALID_RESTORE_REQUEST`; 승인한 contribution manifest와 command outbox를 먼저 저장하고 AI가 현재 본문·링크·embedding을 갱신한 뒤 core 감사 상태를 원자 반영 |
