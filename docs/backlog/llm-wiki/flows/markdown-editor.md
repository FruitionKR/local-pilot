# llmPipeline Markdown 편집기 흐름

## 1. 문서 목적

이 문서는 `/agent/turn`이 사용자 요청을 Markdown 편집·생성·일반 질의로 분류하고, Markdown 결과를 안전하게 생성하는 현재 흐름을 설명한다.
...
- HTTP 진입점: `llmPipeline/app/modules/agent/interfaces/http/routes.py`
- turn orchestration: `llmPipeline/app/modules/agent/application/handle_agent_turn.py`
- turn router: `llmPipeline/app/modules/agent/infrastructure/chat_completions_turn_router.py`
- Markdown LLM adapter: `llmPipeline/app/modules/markdown_edit/infrastructure/chat_completions_markdown_editor.py`
- target scope: `llmPipeline/app/modules/markdown_edit/domain/markdown_target_scope.py`
- 출력 계약: `llmPipeline/app/modules/markdown_edit/domain/markdown_output_contract.py`
- source-range 편집: `llmPipeline/app/modules/markdown_edit/infrastructure/markdown_source_range.py`
...
target type은 `selection`, `current_section`, `whole_document`다. active Markdown이나 target이 없는 상태에서 구체적인 edit를 안전하게 수행할 수 없으면 clarify로 보낸다.

**prompt**

명백한 target 누락이나 수행 불가 요청은 local guard가 LLM 없이 `clarify`/`reject`한다. 그 외에는 router prompt가 `chat_answer`, `markdown_create`, `markdown_edit`, `clarify`, `reject` 중 하나와 edit goal을 JSON으로 반환하도록 요구한다.

router system prompt는 `llmPipeline/prompts/agent_turn_router.system.md`다. user payload에는 message, conversation summary/reference context, active Markdown 존재 여부와 target metadata가 들어간다. router는 Markdown을 직접 수정하지 않고 action 선택만 담당한다.
...
외부 template 적용과 full-document reconstruction은 의도적으로 deferred라는 지시 때문에 `markdown_edit`가 아니라 `clarify + template_transform`이 된다. 즉 router 결과가 이후 LLM을 호출할지, 어떤 출력 계약을 적용할지를 직접 바꾼다.
...
llmPipeline은 허용 action과 필수 field를 검사한다. edit는 target 확정으로, create는 생성 prompt로 가며 나머지는 Markdown operation 없이 메시지를 반환한다.
...
heading, list, code fence 같은 구조 경계를 깨지 않아야 하며 편집 영역 밖 최대 20줄은 읽기 전용 context다.
...
requested target이 Markdown 구조의 중간에서 끝나면 actual target을 안전한 section/block 경계로 확장할 수 있으며 응답의 `scope_expanded`로 드러난다. whole-document fallback은 target을 추론할 수 없는 모든 요청에 자동 적용되는 것이 아니라 router와 scope 정책을 통과해야 한다.
...
구현은 `markdown_output_contract.py`, `markdown_source_range.py`에 있다. `cleanup`, `style_change`, `shorten`은 기본 보호 goal이며 instruction이 구조 자체의 변경을 명시하면 해당 보호를 완화한다.
...
footnote definition과 frontmatter는 단순 문자열 치환 외에 원래 위치·구조를 복구하는 규칙이 있다. token이 0개 또는 2개 이상 반환되면 정상 결과로 간주하지 않는다.
...
일반 edit는 `llmPipeline/prompts/markdown_edit.system.md`, source-range edit는 `llmPipeline/prompts/markdown_source_edit.system.md`를 사용한다. 구현은 `chat_completions_markdown_editor.py`의 일반/source-range 경로로 나뉜다.
...
llmPipeline은 checklist marker를 다시 검사하고 actual target에 대한 `replace` operation으로 조립한다. 모델이 `- API 계약 확인`을 반환하면 문법상 유효한 Markdown이어도 checklist 계약 실패이므로 적용하지 않고 retry한다.

source-range에서 `[공식 문서](https://example.com)`의 표시 문구만 바꾸는 경우에는 prompt가 URL을 볼 필요가 없다. llmPipeline이 URL을 보호하고 `text-0001=공식 문서`만 전달한다. 모델이 `{"id":"text-0001","replacement":"API 문서"}`를 반환하면 segment patch와 token 복원 결과가 `[API 문서](https://example.com)`가 된다.
...
모델은 feedback에 지목된 계약을 고친 전체 JSON을 다시 반환해야 하며 llmPipeline은 두 번째 결과도 처음부터 동일하게 검증한다.
...
- 원문의 citation/footnote 개수가 줄지 않는다.
- 원문에 없던 한자를 한국어 편집 결과가 새로 만들지 않는다.
- literal anchor, URL과 구조 fragment 개수는 유지된다.

일부 기계적으로 확실한 오류는 `repair_markdown_output()`으로 먼저 고친다. 예를 들어 display math를 감싼 불필요한 Markdown fence를 제거할 수 있다.
...
llmPipeline은 이 결과를 저장하지 않는다. 호출자가 현재 문서가 요청 시점과 같은지 확인한 뒤 operation을 적용한다.
...
system prompt는 `llmPipeline/prompts/markdown_create.system.md`다. create와 edit는 동일한 `/agent/turn` endpoint와 schema feature를 공유하지만, create 결과에는 requested/actual target이나 replace operation이 없다.
...
예를 들어 conversation summary에 “API 계약을 먼저 확정한다”, reference context에 “다음 작업은 담당자 지정”만 있다면 모델은 이 두 사실로 제목·요약·Markdown을 만든다. prompt가 미제공 정보 생성을 금지하므로 날짜나 담당자 이름을 임의로 채우지 않는다. 출력의 `title`, `summary`, `markdown`이 비어 있지 않은지 llmPipeline이 검사한 뒤 create 결과로 반환한다.
...
필수 field, 비어 있지 않은 본문과 Markdown 문법을 검사하고 실패 feedback으로 한 번 retry한다. 성공 결과도 llmPipeline이 직접 저장하지 않으며 호출자가 새 문서 생성에 사용한다.
...
- edit 결과는 저장 완료가 아니라 호출자가 적용할 patch 제안이다.
...
- `docs/spec/agent-markdown-contract.md`
