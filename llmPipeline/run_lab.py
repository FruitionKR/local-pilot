...
def _evaluate_source_accumulation(
    *,
    ...
    payload = {
        ...
        "draft": {
            "accumulated_summary_candidates": [n.get("semantic_summary", "") for n in normalized.get("semantic_notes", []) if n.get("semantic_summary")],
            "key_points": [kp for note in normalized.get("semantic_notes", []) for kp in note.get("key_points", [])],
            "observations": normalized.get("observations", []),
            "categories": normalized.get("categories", []),
        },
    }
    ...
    result = _apply_source_accumulation_result(normalized, raw, source_blocks)
    log.emit(
        "5-보조. Source 누적 평가",
...
def _apply_source_accumulation_result(normalized: dict[str, Any], raw: dict[str, Any], source_blocks: list[Any]) -> dict[str, Any]:
