...
public class FolderController {
    ...
    @ApiResponses({
        ...
        @ApiResponse(responseCode = "400", description = "잘못된 이름, 위치 또는 Idempotency-Key"),
        @ApiResponse(responseCode = "404", description = "워크스페이스 또는 상위 폴더를 찾을 수 없음"),
        @ApiResponse(responseCode = "409", description = "이름 또는 멱등 키 충돌")
    })
    @PostMapping
    ...
    @ApiResponses({
        ...
        @ApiResponse(responseCode = "400", description = "잘못된 이름, version 또는 Idempotency-Key"),
        @ApiResponse(responseCode = "404", description = "폴더 또는 워크스페이스를 찾을 수 없음"),
        @ApiResponse(responseCode = "409", description = "이름, version 또는 멱등 키 충돌")
    })
    @PatchMapping("/{folder_id}")
    public ResponseEntity<FolderResponse> rename(
            @PathVariable("workspace_id") String workspaceId,
            @AuthenticationPrincipal String userId,
            @Parameter(description = "이름을 변경할 폴더 ID")
    ...
    @ApiResponses({
        ...
        @ApiResponse(responseCode = "400", description = "허용되지 않는 이동 또는 잘못된 요청"),
    ...
    public ResponseEntity<FolderResponse> move(
            @PathVariable("workspace_id") String workspaceId,
            @AuthenticationPrincipal String userId,
            @Parameter(description = "이동할 폴더 ID")
            @PathVariable("folder_id") UUID folderId,
            @Parameter(description = "요청 멱등 키", required = true)
            @RequestHeader(value = "Idempotency-Key", required = false) String idempotencyKey,
    ...
    @Operation(summary = "폴더 하위 항목 조회", description = "폴더 바로 아래의 하위 폴더와 문서를 혼합 정렬 순서로 반환합니다.")
    @ApiResponses({
    ...
    public ResponseEntity<FolderChildrenResponse> children(
            @PathVariable("workspace_id") String workspaceId,
            @AuthenticationPrincipal String userId,
            @Parameter(description = "조회할 폴더 ID")
            @PathVariable("folder_id") UUID folderId) {
    ...
    @ApiResponses({
        ...
        @ApiResponse(responseCode = "400", description = "잘못된 version 또는 Idempotency-Key"),
        @ApiResponse(responseCode = "404", description = "폴더 또는 워크스페이스를 찾을 수 없음"),
        @ApiResponse(responseCode = "409", description = "version 또는 멱등 키 충돌")
    })
    ...
    public ResponseEntity<FolderLifecycleResponse> delete(
            @PathVariable("workspace_id") String workspaceId,
            @AuthenticationPrincipal String userId,
            @Parameter(description = "삭제할 폴더 ID")
            @PathVariable("folder_id") UUID folderId,
            @Parameter(description = "요청 멱등 키", required = true)
            @RequestHeader(value = "Idempotency-Key", required = false) String idempotencyKey,
    ...
    @Operation(summary = "폴더 복구", description = "삭제된 폴더와 하위 항목의 접근을 복구하고 유효한 탐색 위치에 배치합니다.")
    @ApiResponses({
    ...
    public ResponseEntity<FolderLifecycleResponse> restore(
            @PathVariable("workspace_id") String workspaceId,
            @AuthenticationPrincipal String userId,
            @Parameter(description = "복구할 폴더 ID")
            @PathVariable("folder_id") UUID folderId,
            @Parameter(description = "요청 멱등 키", required = true)
            @RequestHeader(value = "Idempotency-Key", required = false) String idempotencyKey,
            @Valid @RequestBody DocumentLifecycleRequest request) {
