...
- **Reply to every review finding in its original thread with the disposition and evidence.** For a fixed finding, start with `수정했습니다.` and state the actual change boundary and verification. For an unchanged finding, start with `유지했습니다.` and state the governing contract and evidence.
...
**Keep only `docs/README.md`, `docs/{architecture,data-model,script}.md`, `docs/api/`, and `docs/adr/` as current docs. Archive everything else to `docs/backlog/`. Do not create dated issue or changelog docs.**

- 현행 문서는 `docs/README.md`(문서 안내), `docs/architecture.md`(서비스 경계·통신·인가·배포), `docs/api/`(서비스·도메인별 API 계약), `docs/data-model.md`(저장소·테이블 소유권), `docs/script.md`(구동 스크립트·데모 절차)를 유지한다.
